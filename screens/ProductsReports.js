...
import ProductsReportsInfo from '../jsons/ProductsReportsInfo';
...
export default class ProductsReports extends Component {
    constructor(props) {
        super(props);
        this.state = {
            selectDay: true,
                ...
                <View style={{ flex: 2, padding: 5, justifyContent: 'space-around' }}>
                    ...
                    <View style={{ flex: 0.75, backgroundColor: '#ffffff', borderRadius: 16, }}>
                        ...
                        <View style={{ backgroundColor: '#E6E6E6', flex: 0.1, borderTopLeftRadius: 16, borderTopRightRadius: 16, flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center', paddingHorizontal: 5 }}>
                            <Text style={{ flex: 0.16, textAlign: 'center' }}>{Dictionary.ID[lan]}</Text>
                            <Text style={{ flex: 0.16, textAlign: 'center' }}>{Dictionary.NAME[lan]}</Text>
                            <Text style={{ flex: 0.16, textAlign: 'center' }}>{Dictionary.SIZE[lan]}</Text>
                            <Text style={{ flex: 0.16, textAlign: 'center' }}>{Dictionary.CATEGORY[lan]}</Text>
                            <Text style={{ flex: 0.16, textAlign: 'center' }}>{Dictionary.SOLD_QUANTITY[lan]}</Text>
                            <Text style={{ flex: 0.16, textAlign: 'center' }}>{Dictionary.TOTAL_SALES[lan]}</Text>

                        </View>
                        <View style={{ flex: 1 }}>
                            {ProductsReportsInfo.Products.map((item, key) =>
                                (
                                    <View style={{ flexDirection: 'row', justifyContent: 'space-around', borderBottomWidth: 1, borderBottomColor: '#F1F1F1', paddingVertical: 10 }}>
                                        <Text style={{ flex: 0.16, textAlign: 'center' }}>{item.ID}</Text>
                                        <Text style={{ flex: 0.16, textAlign: 'center' }}>{item.name}</Text>
                                        <Text style={{ flex: 0.16, textAlign: 'center' }}>{item.size}</Text>
                                        <Text style={{ flex: 0.16, textAlign: 'center' }}>{item.category}</Text>
                                        <Text style={{ flex: 0.16, textAlign: 'center' }}>{item.soldQuantity}</Text>
                                        <Text style={{ flex: 0.16, textAlign: 'center' }}>{item.totalSales}</Text>
                                    </View>
                                )
                            )
                            }
                        ...
                        <View style={{ flex: 0.4, padding: 20, backgroundColor: '#ffffff', borderRadius: 16, justifyContent: 'space-between' }}>
                            ...
                            <View style={{ borderBottomWidth: 1, }}>
                                ...
                                <View style={{ paddingVertical: 5, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                                    ...
                                    <Text style={[styles.fs_5, { textAlign: 'center', fontWeight: 'bold', color: '#3FAEA3', flex: 0.2 }]}>{ProductsReportsInfo.pickup.paid}</Text>
                                    <Text style={[styles.fs_5, { textAlign: 'center', fontWeight: 'bold', color: '#D92E28', flex: 0.2 }]}>{ProductsReportsInfo.pickup.VOID}</Text>
                                </View>
                                <View style={{ paddingVertical: 5, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <Text style={[styles.fs_5, { fontWeight: 'bold', flex: 0.4, color: '#707070' }]}>{Dictionary.DELIVERY[lan]}</Text>
                                    <Text style={[styles.fs_5, { textAlign: 'center', fontWeight: 'bold', color: '#3FAEA3', flex: 0.2 }]}>{ProductsReportsInfo.delivery.paid}</Text>
                                    <Text style={[styles.fs_5, { textAlign: 'center', fontWeight: 'bold', color: '#D92E28', flex: 0.2 }]}>{ProductsReportsInfo.delivery.VOID}</Text>
                                </View>
                                <View style={{ paddingVertical: 5, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <Text style={[styles.fs_5, { fontWeight: 'bold', flex: 0.4, color: '#707070' }]}>{Dictionary.TABLE[lan]}</Text>
                                    <Text style={[styles.fs_5, { textAlign: 'center', fontWeight: 'bold', color: '#3FAEA3', flex: 0.2 }]}>{ProductsReportsInfo.table.paid}</Text>
                                    <Text style={[styles.fs_5, { textAlign: 'center', fontWeight: 'bold', color: '#D92E28', flex: 0.2 }]}>{ProductsReportsInfo.table.VOID}</Text>
                                </View>
                                <View style={{ paddingVertical: 5, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <Text style={[styles.fs_5, { fontWeight: 'bold', flex: 0.4, color: '#707070' }]}>{Dictionary.APPLICATION[lan]}</Text>
                                    <Text style={[styles.fs_5, { textAlign: 'center', fontWeight: 'bold', color: '#3FAEA3', flex: 0.2 }]}>{ProductsReportsInfo.application.paid}</Text>
                                    <Text style={[styles.fs_5, { textAlign: 'center', fontWeight: 'bold', color: '#D92E28', flex: 0.2 }]}>{ProductsReportsInfo.application.VOID}</Text>
                                </View>
                            </View>
                            <View style={{ alignItems: 'center' }}>
                                <Text style={[styles.fs_8, { fontWeight: 'bold', color: '#707070' }]}>{Dictionary.TOTAL_ORDERS[lan]}</Text>
                                <View style={{ paddingVertical: 5, flexDirection: 'row' }}>
                                    <Text style={[styles.fs_5, { fontWeight: 'bold', color: '#D92E28', paddingHorizontal: 5 }]}>{ProductsReportsInfo.totalOrders}</Text>
                        ...
                        <View style={{ flex: 0.4, backgroundColor: '#ffffff', borderRadius: 16 }}>
                            <View style={{ flex: 0.8, alignItems: 'center', justifyContent: 'center' }}>
                                ...
                                <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
                                    <Text style={[styles.fs_12, { padding: 10, color: '#3FAEA3' }]}>{ProductsReportsInfo.totalSales}</Text>
                                    <Text style={[styles.fs_8, { color: '#707070' }]}>{ProductsReportsInfo.curruncy[lan]}</Text>
                                </View>
