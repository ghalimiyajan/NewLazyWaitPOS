...
export default class Reports extends Component {
    ...
    // selectDayF() {
    //     this.setState(!this.state.selectDay)
    // }
                ...
                <View style={{ flex: 1, borderRadius: 16, padding: 20, }}>
                    ...
                    <View style={[styles.whiteSidebar, { justifyContent: 'space-around' }]}>
                        ...
                        <View style={{ flex: 0.1, flexDirection: 'row', alignItems: 'center' }}>
                            <TouchableOpacity onPress={() => this.setState(selectDay => ({ selectDay: !this.state.selectDay }))} >
                            ...
                            <TouchableOpacity onPress={() => this.setState(selectWeek => ({ selectWeek: !this.state.selectWeek }))}>
                            ...
                            <TouchableOpacity onPress={() => this.setState(selectMonth => ({ selectMonth: !this.state.selectMonth }))}>
                            ...
                            <TouchableOpacity onPress={() => this.setState(selectCustom => ({ selectCustom: !this.state.selectCustom }))}>
                        ...
                        <View style={{ flex: 0.4, padding: 20, backgroundColor: '#ffffff', borderRadius: 16, justifyContent: 'space-between' }}>
                            ...
                            <View style={{ borderBottomWidth: 1, }}>
                                <View style={{ paddingVertical: 5, flexDirection: 'row', justifyContent: 'space-between' }}>
                                    <View style={{ flex: 0.6 }} />
                                    <Text style={[styles.fs_5, { fontWeight: 'bold', color: '#3FAEA3', flex: 0.33 }]}>{Dictionary.PAID[lan]}</Text>
                                    <Text style={[styles.fs_5, { fontWeight: 'bold', color: '#D92E28', flex: 0.33 }]}>{Dictionary.VOID[lan]}</Text>

                                </View>
                                <View style={{ paddingVertical: 5, flexDirection: 'row', justifyContent: 'space-between' }}>
                                    <Text style={[styles.fs_5, { fontWeight: 'bold', flex: 0.4, color: '#707070' }]}>{Dictionary.PICKUP[lan]}</Text>
                                    <Text style={[styles.fs_5, { fontWeight: 'bold', color: '#3FAEA3', flex: 0.2 }]}>{billsReportInfo.pickup.paid}</Text>
                                    <Text style={[styles.fs_5, { fontWeight: 'bold', color: '#D92E28', flex: 0.2 }]}>{billsReportInfo.pickup.VOID}</Text>
                                </View>
                                <View style={{ paddingVertical: 5, flexDirection: 'row', justifyContent: 'space-between' }}>
                                    <Text style={[styles.fs_5, { fontWeight: 'bold', flex: 0.4, color: '#707070' }]}>{Dictionary.DELIVERY[lan]}</Text>
                                    <Text style={[styles.fs_5, { fontWeight: 'bold', color: '#3FAEA3', flex: 0.2 }]}>{billsReportInfo.delivery.paid}</Text>
                                    <Text style={[styles.fs_5, { fontWeight: 'bold', color: '#D92E28', flex: 0.2 }]}>{billsReportInfo.delivery.VOID}</Text>
                                </View>
                                <View style={{ paddingVertical: 5, flexDirection: 'row', justifyContent: 'space-between' }}>
                                    <Text style={[styles.fs_5, { fontWeight: 'bold', flex: 0.4, color: '#707070' }]}>{Dictionary.TABLE[lan]}</Text>
                                    <Text style={[styles.fs_5, { fontWeight: 'bold', color: '#3FAEA3', flex: 0.2 }]}>{billsReportInfo.table.paid}</Text>
                                    <Text style={[styles.fs_5, { fontWeight: 'bold', color: '#D92E28', flex: 0.2 }]}>{billsReportInfo.table.VOID}</Text>
                                </View>
                                <View style={{ paddingVertical: 5, flexDirection: 'row', justifyContent: 'space-between' }}>
                                    <Text style={[styles.fs_5, { fontWeight: 'bold', flex: 0.4, color: '#707070' }]}>{Dictionary.APPLICATION[lan]}</Text>
                                    <Text style={[styles.fs_5, { fontWeight: 'bold', color: '#3FAEA3', flex: 0.2 }]}>{billsReportInfo.application.paid}</Text>
                                    <Text style={[styles.fs_5, { fontWeight: 'bold', color: '#D92E28', flex: 0.2 }]}>{billsReportInfo.application.VOID}</Text>
                                </View>
                            </View>
                            <View style={{ alignItems: 'center' }}>
